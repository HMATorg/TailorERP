...
/**
 * Fresh-checkout confirmation (v4 §1 amendment, D-047). The three printable
 * documents themselves — thermal receipt, garment tags, A4 tax invoice — live
 * in the shared PrintCenter component (D-051) so this page and the order
 * history reprint view can never render them differently.
 *
    ...
    return (
      <div style={{ padding: 24 }}>
        <Alert
          ...
          description="Reopen this order from Orders to reprint."
        />
        <Space style={{ marginBlockStart: 12 }}>
          <Button onClick={() => navigate('/')}>New order</Button>
          {id && <Button type="primary" onClick={() => navigate(`/orders/${id}`)}>Open order</Button>}
        </Space>
      </div>
    );
  }
          ...
          <Space align="center" style={{ marginBlockEnd: 16 }}>
            ...
            <div>
              <Typography.Title level={3} style={{ margin: 0 }}>
                Order {state.orderNumber} created {state.isUrgent && <Tag color="red">URGENT</Tag>}
              </Typography.Title>
